...
for obj_level in obj_level_list:
    ...
    try:
        ...
        con = cx_Oracle.connect(
            "maximo", "maximo", "localhost:1521/max1", encoding="UTF-8")
        cur = con.cursor()
        ...
        for dict_object in mx_obj_list:
            ...
            for result in cur.execute(select_statement):
                ...
                for item in result:
                    ...
                    attr_sequence = attr_seq_dict.get(select_attr)

                    if attr_sequence:

                        mapped_value = attr_sequence + ".NEXTVAL"

                    attr_site_org = attr_siteorg.get(select_attr)
                    if item and attr_site_org:
                        if select_attr == 'ORGID' or attr_site_org == 'ORGID':
                            item = item.replace(old_org, new_org)
                        elif select_attr == 'SITEID' or attr_site_org == 'SITEID':
                            item = item.replace(old_site, new_site)
